...
> 	- $V_{\bar{0}}$ is the even subspace as $\bar{0}$ is the set of all integers that correspond to $0$ in $\text{mod}(2)$ - this is also known as the bosonic or even subspace.
> 	- $V_{\bar{1}}$ is hence, the odd subspace, $\bar{1}$ is then the set of all integers that corresponds to $1$ in $\text{mod}(2)$ - this is also known as the fermionic or odd subspace.
...
> - The [[Functors|tensor product]] of two of these $sVect$ is given as:
...
> 
> - We will also have a super commutativity relation for the symmetry [[Morphisms|isomorphism]] $T:V\otimes W\to W\otimes V$ is given by: $$T(v\otimes w)=(-1)^{\text{deg}(v)\cdot\text{deg}(w)}(w\otimes v)$$
 ^def-vect-svs
